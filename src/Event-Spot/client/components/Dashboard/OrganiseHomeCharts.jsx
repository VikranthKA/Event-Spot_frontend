...
const OrganiseHomeCharts = () => {
  ...
  function getData(){
    ...
    // Define chart data
    setChartData({
      labels: [`Avg Ticket-Cost : ${averageTicketCost/orgEvents.length}`, `totalTicketsSold : ${totalTicketSold}`, `Avg Review : ${Math.ceil(avgReviews/orgEvents.length)}`],
      datasets: [
        {
          ...
          data: [averageTicketCost/orgEvents.length,totalTicketSold,Math.ceil(avgReviews/orgEvents.length)]
        }
      ]
    });
  }
